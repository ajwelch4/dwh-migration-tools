...
class MacroProcessorArgsSchema(Schema):
    """Schema and data validator for MacroProcessor args."""
...
class MacroProcessor(Processor):
    ...
    def __init__(self, macro_expansion_maps: Dict[str, Dict[str, str]]) -> None:
        self.macro_expansion_maps = macro_expansion_maps
        self.reversed_maps = self._get_reversed_maps()

    @classmethod
    def from_args(cls, args: Dict[str, object]) -> "MacroProcessor":
        try:
            validated_args: Dict[
                str, Dict[str, Dict[str, str]]
            ] = MacroProcessorArgsSchema().load(args)
        except ValidationError as error:
            logging.error("Invalid MacroProcessor args: %s: %s.", args, error)
            raise
        logging.info(
            "MacroProcessor args:\n%s.",
            pformat(args),
        )
        return cls(validated_args["macros"])
