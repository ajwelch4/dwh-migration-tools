...
class GcsProcessorArgsSchema(Schema):
    """Schema and data validator for GcsProcessor args."""
...
class GcsProcessor(Processor):
    ...
    @classmethod
    def from_args(cls, args: Dict[str, object]) -> "GcsProcessor":
        try:
            validated_args: Dict[str, str] = GcsProcessorArgsSchema().load(args)
        except ValidationError as error:
            logging.error("Invalid GcsProcessor args: %s: %s.", args, error)
            raise
        logging.info(
            "GcsProcessor args:\n%s.",
            pformat(args),
        )
        return cls(validated_args["bucket"])
    ...
    def postprocess(self, relative_path: pathlib.Path, text: str) -> str:
        ...
        logging.info(
            "Downloading from GCS: %s/%s.",
            self.download_uri,
            relative_path,
        )
        translated_text: str = blob.download_as_text()
        logging.info(
            "Finished downloading from GCS: %s/%s.",
            self.download_uri,
            relative_path,
        )
